    ...
    case REG_TYPE_0X:
        {
            if(1 == mAddrInfo.mAddrProp.nAddrLen){
                ...
                /*开始地址 vector index = 2*/
                int nStartAddr = mAddrInfo.mAddrProp.nAddrValue;
                nTmpValue = (nStartAddr >> 8) & 0xff;
                mSendPackge.sendDataList.push_back(nTmpValue);

                /*vector index = 3*/
                nTmpValue = nStartAddr & 0xff;
                mSendPackge.sendDataList.push_back(nTmpValue);

                /*元件个数 vector index=4*/
                int nBitNum = mAddrInfo.mAddrProp.nAddrLen;
                if(nBitNum > mAddrInfo.nValueList.size()){
                    nBitNum = mAddrInfo.nValueList.size();
                }
                nTmpValue = (nBitNum >> 8) & 0xff;
                mSendPackge.sendDataList.push_back(nTmpValue);

                /*vector index = 5*/
                nTmpValue = nBitNum & 0xff;
                mSendPackge.sendDataList.push_back(nTmpValue);

                /* 字节数 vector index = 6*/
                int nByteNum = nBitNum/8+(nBitNum%8==0?0:1);
                nTmpValue = nByteNum & 0xff;
                mSendPackge.sendDataList.push_back(nTmpValue);
                ...
                for(int i=0;i<nByteNum;i++){
                    ...
                    for(int j=0;j<8;j++){
                        int index = i*8+j;
                        if(index < nBitNum){
                            if(mAddrInfo.nValueList.at(index) == 1){
                                nTmpValue |= 1<<j;
                            }
                        }
                    }
...
bool PlcProtocol::sortOutAddrList(vector<ADDR_TARGET_PROP> mSourceAddrList, vector<ADDR_TARGET_PROP> &targetList, int nMaxRWlen, bool bWriteAddr)
{
    ...
    {
        /*写地址，则只支持长度为1的写*/
        mTmpListBit0 = PlcMathMethod::addrSortOut(mTmpListBit0, nMaxRWlen, 8 , false, true);
    }
    else
    {
        /*读地址，可支持多个长度*/
