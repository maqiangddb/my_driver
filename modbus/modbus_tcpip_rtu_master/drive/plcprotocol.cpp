    ...
    case CMN_READ_BEFORE_WRITE:
        {
            if(REG_TYPE_4X_BIT == eRegType)
            {
                ...
                /* index = 6, 站号 */
                uchar nTmpValue = mAddrInfo.mAddrProp.nPlcStationIndex & 0xff;
                ...
                /*返回数据的长度*/
                mSendPackge.nReturnLen = nAddrLen *2 + 9;
            }
            break;
        }
...
bool PlcProtocol::makeReadPackage(CMN_NEED_PROP mCmnProp, unsigned int &nCmnTimes, SEND_PACKAGE &mSendPackge)
{
    ...
    /* index = 6, 站号*/
    uchar nTmpValue = mAddrInfo.nPlcStationIndex & 0xff;
    mSendPackge.sendDataList.push_back(nTmpValue);
    ...
    {
        mSendPackge.nReturnLen = nReturnLen + 9;
    }
    ...
    case REG_TYPE_4X_DB:
        {
            ...
            {
                ushort nTmpUSValue = 7 + mAddrInfo.mAddrProp.nAddrLen*2;

                /* index = 4, 长度字段 （上半部分字节） = 0 (所有的消息长度小于256)*/
                mSendPackge.sendDataList.push_back((nTmpUSValue >> 8) & 0xff);

                /* index = 5, 长度字段  (下半部分字节)   = 后面字节的数量*/
                mSendPackge.sendDataList.push_back(nTmpUSValue & 0xff);
