...
bool DataCenter::setReadData(ADDR_RW_STRUCT mAddrInfo, STATION_VALUE_PROP &readDataList, bool bUserData, CMN_RW_ERROR_CODE eSuccess)
{
    ...
    case REG_TYPE_CTH:
        {
            /*数据存储区的容器不够则重新分配空间*/
            int nSize = readDataList.regWordList[eRegType - 3].size();
            if(nSize < mAddr.nAddrValue + mAddr.nAddrLen)
            {
                readDataList.regWordList[eRegType - 3].resize(mAddr.nAddrValue + mAddr.nAddrLen, emptyWordValue);
            }
            ...
            if(bUserData)  //是用户设置的值
            {
                ...
                {
                    /*设置状态*/
                    readDataList.regWordList[eRegType - 3][mAddr.nAddrValue + i].eErrorStatus = eSuccess;
                    ...
                    nTmpValue += nLowByte;
                    readDataList.regWordList[eRegType - 3][mAddr.nAddrValue + i].nValue = nTmpValue;
                }
            }
            else      //如果是从PLC读取的数据
            {
                if(nSetLen > nListSize/5)
                {
                    nSetLen = nListSize/5;
                }
                for(int i = 0; i < nSetLen; i++)
                {
                    ...
                    /*设置值*/
                    unsigned short usTemp1 = valueList.at(i*5 + 0);
                    unsigned short usTemp2 = valueList.at(i*5 + 1);
                    unsigned short usTemp3 = valueList.at(i*5 + 2);
                    unsigned short usTemp4 = valueList.at(i*5 + 3);
                    unsigned short usTemp5 = valueList.at(i*5 + 4);

                    usTemp1 = PlcMathMethod::asciiToIntValue(usTemp1) * 10000;
                    usTemp2 = PlcMathMethod::asciiToIntValue(usTemp2) * 1000;
                    usTemp3 = PlcMathMethod::asciiToIntValue(usTemp3) * 100;
                    usTemp4 = PlcMathMethod::asciiToIntValue(usTemp4) * 10;
                    usTemp5 = PlcMathMethod::asciiToIntValue(usTemp5) * 1;

                    unsigned short nTmpValue = usTemp1 + usTemp2 + usTemp3 + usTemp4 + usTemp5;
                    readDataList.regWordList[eRegType - 3][mAddr.nAddrValue + i].nValue = nTmpValue;
                }
            }
            break;
    ...
    case REG_TYPE_CTH:
        {
            ...
            {
                /*设置状态*/
                mTmpValueProp.eErrorStatus = readDataList.regWordList[eRegType - 3].at(mAddr.nAddrValue + i).eErrorStatus;

                /*设置值*/
                nTmpValue = readDataList.regWordList[eRegType -3].at(mAddr.nAddrValue + i).nValue;
