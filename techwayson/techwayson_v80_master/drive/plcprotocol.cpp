    ...
    case REG_TYPE_4X:
        {
            ...
            /*判断长度是否超限*/
            if(mAddrTarget.nAddrValue + mAddrTarget.nAddrLen > ADDR_MAX_VALUE_65536 + 1)
            {
                return false;
            }
    ...
    case REG_TYPE_4X:
        {
            ...
            /*不能为负数*/
            if(nStartValue < 1 || nStartValue > ADDR_MAX_VALUE_65536)
            {
                return ILLEGAL_VALUE;
            }
            ...
            for(int i = 0; i < nCopyNum; i++)
            {
                ...
                if(nStartValue + mTmpAddr.nAddrLen <= ADDR_MAX_VALUE_65536 + 1)
                {
                    /*添加整数部分*/
                    memset(buf, 0, 10);
    ...
    case REG_TYPE_0X:
        {
            ...
            iSendAdd = mAddrInfo.mAddrProp.nAddrValue;
