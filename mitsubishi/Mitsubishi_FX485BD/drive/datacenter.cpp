    ...
    case REG_TYPE_S:
        {
            ...
            else//如果是从PLC读取的数据
            {
                ...
                {
                    readDataList.regBitList[eRegType].resize(mAddr.nAddrValue + nListSize *4, emptyBitValue);
                }
                ...
                {
                    uchar nTmpValue1 = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 2)) << 4) & 0xf0;
                    uchar nTmpValue2 = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 3)) << 0) & 0x0f;
                    uchar nTmpValue = nTmpValue1 + nTmpValue2;

                    for(int j = 0; j < 8; j++)
                    {
                        readDataList.regBitList[eRegType][nStartAddr + (2*i)*8 + j].eErrorStatus = eSuccess;
                        if(nTmpValue & (1 << j))
                        {
                            readDataList.regBitList[eRegType][nStartAddr + (2*i)*8 + j].nValue = 1;
                        }
                        else
                        {
                            readDataList.regBitList[eRegType][nStartAddr + (2*i)*8 + j].nValue = 0;
                        }
                    }

                    nTmpValue1 = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 0)) << 4) & 0xf0;
                    nTmpValue2 = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 1)) << 0) & 0x0f;
                    nTmpValue = nTmpValue1 + nTmpValue2;

                    for(int j = 0; j < 8; j++)
                    {
                        readDataList.regBitList[eRegType][nStartAddr + (2*i + 1)*8 + j].eErrorStatus = eSuccess;
                        if(nTmpValue & (1 << j))
                        {
                            readDataList.regBitList[eRegType][nStartAddr + (2*i + 1)*8 + j].nValue = 1;
                        }
                        else
                        {
                            readDataList.regBitList[eRegType][nStartAddr + (2*i + 1)*8 + j].nValue = 0;
                        }
                    }
                }
            }
    ...
    case REG_TYPE_C:
        {
            ...
            else//如果是从PLC读取的数据
            {
                ...
                {
                    uchar nTmpValue = (PlcMathMethod::asciiToIntValue(valueList.at(i))) & 0xff;

                    if(0x01 & nTmpValue)
                    {
                        readDataList.regBitList[eRegType][nStartAddr + i].nValue = 1;
                    }
                    else
                    {
                        readDataList.regBitList[eRegType][nStartAddr + i].nValue = 0;
                    }
//                    for(int j = 0; j < 8; j++)
//                    {
    ...
    case REG_TYPE_D_BIT:
        {
            ...
            else//如果是从PLC读取的数据
            {
                ...
                for(int i = 0; i < nSetDataLen; i++)
                {
                    ...
                    /*设置值 4 0 12 8*/
                    ushort n12Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 0)) << 12) & 0xf000;
                    ushort n8Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 1)) << 8) & 0x0f00;
                    ushort n4Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 2)) << 4) & 0x00f0;
                    ushort n0Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 3)) << 0) & 0x000f;
    ...
    case REG_TYPE_R:
        {
            ...
            else//如果是从PLC读取的数据
            {
                for(int i = 0; i < nListSize/4; i++)
                {
                    ...
                    /*设置值 4 0 12 8*/
                    ushort n4Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 0)) << 12) & 0xf000;
                    ushort n0Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 1)) << 8) & 0x0f00;
                    ushort n12Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 2)) << 4) & 0x00f0;
                    ushort n8Bit = (PlcMathMethod::asciiToIntValue(valueList.at(4*i + 3)) << 0) & 0x000f;
                    nTmpValue = n12Bit + n8Bit + n4Bit + n0Bit;
                    readDataList.regWordList[eRegType - 8][mAddr.nAddrValue + i].nValue = nTmpValue;
                }
            }
    ...
    case REG_TYPE_32CV:
        {
            ...
            else      //如果是从PLC读取的数据
            {
                for(int i = 0; i < nListSize/8; i++)
                {
                    ...
                    /*设置值,低字节在前 高字节在后*/
                    uint nTmp1 = valueList.at(8*i + 0);
                    uint nTmp2 = valueList.at(8*i + 1);
                    uint nTmp3 = valueList.at(8*i + 2);
                    uint nTmp4 = valueList.at(8*i + 3);

                    uint nTmp5 = valueList.at(8*i + 4);
                    uint nTmp6 = valueList.at(8*i + 5);
                    uint nTmp7 = valueList.at(8*i + 6);
                    uint nTmp8 = valueList.at(8*i + 7);


                    nTmp1 = (PlcMathMethod::asciiToIntValue(nTmp1) << 28) & 0xf0000000;
                    nTmp2 = (PlcMathMethod::asciiToIntValue(nTmp2) << 24) & 0x0f000000;
                    nTmp3 = (PlcMathMethod::asciiToIntValue(nTmp3) << 20) & 0x00f00000;
                    nTmp4 = (PlcMathMethod::asciiToIntValue(nTmp4) << 16) & 0x000f0000;
                    nTmp5 = (PlcMathMethod::asciiToIntValue(nTmp5) << 12) & 0x0000f000;
                    nTmp6 = (PlcMathMethod::asciiToIntValue(nTmp6) << 8) & 0x00000f00;
                    nTmp7 = (PlcMathMethod::asciiToIntValue(nTmp7) << 4) & 0x000000f0;
                    nTmp8 = (PlcMathMethod::asciiToIntValue(nTmp8) << 0) & 0x0000000f;

                    nTmpValue = nTmp1 + nTmp2 + nTmp3 + nTmp4 + nTmp5 + nTmp6 + nTmp7 + nTmp8;
                    readDataList.regDoubleWordList[0][mAddr.nAddrValue + i].nValue = nTmpValue;
                }
            }
            break;
