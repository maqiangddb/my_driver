 ...
 * Return: 生成的包
 ******************************************************************/
DLL_EXPORT bool makePackage(CMN_NEED_PROP mCmnProp, uint &nCmnTimes, SEND_PACKAGE &mSendPackge, ADDR_RW_STRUCT &mCurrAddr)
{
    return g_plcProtocolObj.makePackage(mCmnProp, nCmnTimes, mSendPackge, mCurrAddr);
}

/******************************************************************
 * Function: 对从通信口读取到的数据进行解包
 * Parameters: null
 * Return: true or false
 ******************************************************************/
DLL_EXPORT CMN_RW_ERROR_CODE unpackReadData(UNPACK_NEED_PROP &mUnpackProp, vector<uchar> rcvList, vector<uchar > &targetList)
{
    return g_plcProtocolObj.unpackReadData(mUnpackProp, rcvList, targetList);
}

/******************************************************************
 * Function: 检查写数据或读取的数据是否检验正确
 * Parameters: null
 * Return: true or false
 ******************************************************************/
DLL_EXPORT CMN_RW_ERROR_CODE checkRcvData(UNPACK_NEED_PROP &mUnpackProp, vector<uchar> rcvList, CHECK_TYPE eCheckType)
{
    return g_plcProtocolObj.checkRcvData(mUnpackProp, rcvList, eCheckType);
}
