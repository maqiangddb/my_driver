    ...
    case REG_TYPE_Q:
        {
            ...
            if(strList.size() == 2)
            {
                ...
                /*小数部分不能超过16，也不能为负数*/
                if(nIntValue < 0 || nIntValue >= ADDR_MAX_VALUE_16 || nDecValue < 0 || nDecValue >= 8)
                {
                    return false;
                }
                ...
                /*判断长度是否超限*/
                if(mAddrTarget.nAddrValue + mAddrTarget.nAddrLen > ADDR_MAX_VALUE_16 * 8)
                {
                    return false;
                }
                return true;
    ...
    case REG_TYPE_Q:
        {
            ...
            if(strList.size() == 2)
            {
                ...
                for(int i = 0; i < nCopyNum; i++)
                {
                    ...
                    if(nStartValue + mTmpAddr.nAddrLen < ADDR_MAX_VALUE_16*8)
                    {
                        /*添加整数部分*/
                        memset(buf, 0, 10);
...
bool PlcProtocol::getAddrCanRead(ADDR_TARGET_PROP mAddr)
{
    ...
    case REG_TYPE_Q:
        {
            if(mAddr.nAddrValue < 0 || mAddr.nAddrValue + mAddr.nAddrLen > ADDR_MAX_VALUE_16 * 8)
            {
                return false;
            }
            return true;
    ...
    case REG_TYPE_M:
        {
            if(mAddr.nAddrValue < 0 || mAddr.nAddrValue + mAddr.nAddrLen > ADDR_MAX_VALUE_4096 * 8)
            {
                return false;
            }
            return true;
    ...
    case REG_TYPE_AQW:
        {
            if(mAddr.nAddrValue < 0 || mAddr.nAddrValue + mAddr.nAddrLen > ADDR_MAX_VALUE_64)
            {
                return false;
            }
            return true;
