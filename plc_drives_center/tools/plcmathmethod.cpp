 ...
 * Return: 转换的结果
*************************************************************************/
vector<ADDR_TARGET_PROP > PlcMathMethod::addrSortOut(vector<ADDR_TARGET_PROP> sourceList, int nMaxLen, int nNarrow, bool bByteReg)
{
...
*************************************************************************/
bool PlcMathMethod::compareAddr(ADDR_TARGET_PROP oneAddr, ADDR_TARGET_PROP otherAddr)
{
    if(oneAddr.eAddrRWprop != otherAddr.eAddrRWprop)
    {
        return false;
    }
    else if(oneAddr.nAddrLen != otherAddr.nAddrLen)
    {
        return false;
    }
    else if(oneAddr.nAddrValue != otherAddr.nAddrValue)
    {
        return false;
    }
    else if(oneAddr.nPlcStationIndex != otherAddr.nPlcStationIndex)
    {
        return false;
    }
    else if(oneAddr.nRegIndex != otherAddr.nRegIndex)
    {
        return false;
    }
 ...
 * Return: -1 or index
*************************************************************************/
int PlcMathMethod::getbContainAddr(ADDR_RW_STRUCT addrProp, vector<ADDR_RW_STRUCT> list)
{
    int nListSize = list.size();
    if(nListSize <= 0) return -1;
    for(int i = 0; i < nListSize; i++)
    {
        if(compareAddr(list.at(i).mAddrProp,addrProp.mAddrProp))
        {
            return i;
        }
    }
 ...
 * Return: true or false
*************************************************************************/
bool PlcMathMethod::getbContainWAddr(ADDR_RW_STRUCT addrProp, deque<ADDR_RW_STRUCT> &list, bool bCoverDataList)
{
    int nListSize = list.size();
    if(nListSize <= 1) return false;
    for(int i = 0; i < nListSize; i++)
    {
        if(compareAddr(list.at(i).mAddrProp,addrProp.mAddrProp))
        {
            if(addrProp.nValueList.size() == 1 &&
               (addrProp.nValueList.at(0) == 0 || addrProp.nValueList.at(0) == 1) &&
               addrProp.nValueList == list[i].nValueList)
            {
                return false;
            }
            else
            {
                if(bCoverDataList)
                {
                    list[i].nValueList = addrProp.nValueList;
                }
                return true;
            }
        }
