    ...
    case REG_TYPE_Z:
        {
            ...
            /*判断长度是否超限*/
            if(mAddrTarget.nAddrValue + mAddrTarget.nAddrLen > 16)
            {
                return false;
            }
            return true;
    ...
    case WORT_ADDR_REG:
        {
            ...
            sRegNameList.push_back("D_double");
            sRegNameList.push_back("CV_double");
            break;
        }
    case DWORT_ADDR_REG:
        {
    ...
    case REG_TYPE_Z:
        {
            ...
            /*不能为负数*/
            if(nStartValue < 0 || nStartValue >= 16)
            {
                return ILLEGAL_VALUE;
            }
            ...
            for(int i = 0; i < nCopyNum; i++)
            {
                ...
                if(nStartValue + mTmpAddr.nAddrLen <= 16)
                {
                    /*添加整数部分*/
                    memset(buf, 0, 10);
                    sprintf(buf, "%d", nStartValue);
    ...
    case REG_TYPE_C:
        {
            ...
            {
            case REG_TYPE_X:
                iOffsetAdd = 0x04B0;                                        //1200～01455
                break;
            case REG_TYPE_Y:
                iOffsetAdd = 0x0000;                                        //0000～0255(十进制)
                break;
            case REG_TYPE_M:
                iOffsetAdd = 0x07D0;                                        //2000～3999
                break;
            case REG_TYPE_SM:
                iOffsetAdd = 0x1130;                                        //4400～4655
                break;
            case REG_TYPE_S:
                iOffsetAdd = 0x1770;                                        //6000～6991
                break;
            case REG_TYPE_T:
                iOffsetAdd = 0x1F40;                                        //8000～8255
                break;
            case REG_TYPE_C:
                iOffsetAdd = 0x23F0;                                        //9200～9455
                break;
            default:
                return false;
                break;
    ...
    case REG_TYPE_DCV:
        {
            ...
            case REG_TYPE_D:
            case REG_TYPE_DD:
                iOffsetAdd = 0x0000;                                        //0000～7999(十进制)
                break;
            case REG_TYPE_SD:
                iOffsetAdd = 0x1F40;                                        //8000～8255
                break;
            case REG_TYPE_TV:
                iOffsetAdd = 0x2328;                                        //9000～9255
                break;
            case REG_TYPE_CV:
            case REG_TYPE_DCV:
                iOffsetAdd = 0x251C;                                        //9500～9699 CV32:9700～9811
                break;
            case REG_TYPE_Z:
                iOffsetAdd = 0x2134;                                        //8500～8515
                break;
            default:
                return false;
                break;
    ...
    case REG_TYPE_DCV:
        {
            ...
            else if(mAddrInfo.mAddrProp.nAddrLen > 1)
            {
                ...
                if (eRegType == REG_TYPE_DD || eRegType == REG_TYPE_DCV)
                {
...
bool PlcProtocol::getAddrCanRead(ADDR_TARGET_PROP mAddr)
{
    ...
    case REG_TYPE_Z:
        {
            if(mAddr.nAddrValue < 0 || mAddr.nAddrValue + mAddr.nAddrLen > 16)
            {
                return false;
            }
            return true;
...
REG_TYPE_INDEX PlcProtocol::getRegTypeFromInt(int nRegIndex)
{
    ...
    case 12:
        {
            eRegType = REG_TYPE_DD;
            break;
        }
    case 13:
        {
            eRegType = REG_TYPE_DCV;
            break;
        }
    default:
        {
...
bool PlcProtocol::getRegIndexFromName(string sRegName, int &nRegIndex)
{
    ...
    else if((DealWithString::compare("D_double", sRegName)))
    {
        nRegIndex = 12;
        return true;
    }
    else if((DealWithString::compare("CV_double", sRegName)))
    {
        nRegIndex = 13;
        return true;
    }
