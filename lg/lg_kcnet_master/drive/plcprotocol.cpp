...
CMN_RW_ERROR_CODE PlcProtocol::unpackReadData(UNPACK_NEED_PROP &mUnpackProp, vector<unsigned char> rcvList, vector<unsigned char> &targetList)
{
    ...
    if(mUnpackProp.bNeedSetData)
    {
        ...
        if (mUnpackProp.mCurrAddr.mAddrProp.nRegIndex == REG_TYPE_T ||
            mUnpackProp.mCurrAddr.mAddrProp.nRegIndex == REG_TYPE_C)
        {
            for(int i = 8; i < nRcvSize; i++)
            {
    ...
    case REG_TYPE_F:
        {
            ...
            mSendPackge.sendDataList.push_back(0x30);//7个地址符
            ...
            ushort nCheckResult = PlcMathMethod::getSumCheck(mSendPackge.sendDataList);
            ...
            mSendPackge.nReturnLen = 13 + nAddrLen*4;
            break;
        }
    case REG_TYPE_D:
    case REG_TYPE_TV:
    case REG_TYPE_CV:
        {
            ...
            nTmpValue = PlcMathMethod::intToAsciiValue((mAddrInfo.nAddrLen >> 4) & 0xf);
            mSendPackge.sendDataList.push_back(nTmpValue);

            nTmpValue = PlcMathMethod::intToAsciiValue((mAddrInfo.nAddrLen >> 0) & 0xf);
            mSendPackge.sendDataList.push_back(nTmpValue);
            ...
            mSendPackge.nReturnLen = 13 + mAddrInfo.nAddrLen*4;
            break;
        }
    case REG_TYPE_T:
    case REG_TYPE_C:
        {
            ...
            nAddrValue = (mAddrInfo.nAddrValue/8)*8;//起始地址
            for (int i=0; i<8; i++)
            {
                mSendPackge.sendDataList.push_back(0x30);//7个地址符
                mSendPackge.sendDataList.push_back(0x37);
                ...
                nTmpValue = nAddrValue + i;
                uchar uctmp1 = (nTmpValue/1000) & 0xf;
                uchar uctmp2 = ((nTmpValue%1000)/100) & 0xf;
                uchar uctmp3 = ((nTmpValue%100)/10) & 0xf;
                uchar uctmp4 = (nTmpValue%10) & 0xf;
    ...
    case REG_TYPE_C:
        {
            ...
            {
                uchar uctmp1 = (mAddrInfo.mAddrProp.nAddrValue/100) & 0xf;
                uchar uctmp2 = ((mAddrInfo.mAddrProp.nAddrValue%100)/10) & 0xf;
                uchar uctmp3 = (mAddrInfo.mAddrProp.nAddrValue%10) & 0xf;
            ...
            else
            {
                ...
                uchar uctmp1 = (nWordPass/100) & 0xf;
                uchar uctmp2 = ((nWordPass%100)/10) & 0xf;
                uchar uctmp3 = (nWordPass%10) & 0xf;
...
bool PlcProtocol::sortOutAddrList(vector<ADDR_TARGET_PROP> mSourceAddrList, vector<ADDR_TARGET_PROP> &targetList, int nMaxRWlen, bool bWriteAddr)
{
    ...
    {
        /*读地址，可支持多个长度*/
        mTmpListBit5 = PlcMathMethod::addrSortOut(mTmpListBit5, 1, 8);
    }
    ...
    {
        /*读地址，可支持多个长度*/
        mTmpListBit6 = PlcMathMethod::addrSortOut(mTmpListBit6, 1, 8);
    }
