    ...
    case REG_TYPE_C:
        {
            ...
            else       //如果是从PLC读取的数据
            {
                ...
                {
                    readDataList.regBitList[eRegType - 5].resize(mAddr.nAddrValue + nListSize*4, emptyBitValue);
                }

                valueList.resize(2);
                uchar uctmp = PlcMathMethod::asciiToIntValue(valueList.at(1));

                if(uctmp)
                {
                    readDataList.regBitList[eRegType - 5][mAddr.nAddrValue].nValue = 1;
                }
                else
                {
                    readDataList.regBitList[eRegType - 5][mAddr.nAddrValue].nValue = 0;
                }
            }
            break;
        }
