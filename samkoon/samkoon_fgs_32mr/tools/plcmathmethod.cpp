 ...
 * Return: 转换的结果
*************************************************************************/
vector<ADDR_TARGET_PROP > PlcMathMethod::addrSortOutForCV(vector<ADDR_TARGET_PROP> sourceList, int nMaxLen, int nNarrow, bool bByteReg)
{
    if(sourceList.empty()) return sourceList;
    if(nMaxLen <= 0 || nNarrow <= 0) return sourceList;
    ...
    for(int i = 1; i < nSourceSize; i++)
    {
        ...
        int nNewLen = (tmpAddrValue.nAddrValue - nLastValue)*2 + tmpAddrValue.nAddrLen;
        if(tmpAddrValue.nAddrValue < nLastValue)
        {
            tmpAddrValue.nAddrLen = nNewLen;
            tmpAddrValue.nAddrValue = nLastValue;
