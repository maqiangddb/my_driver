    ...
    case REG_TYPE_SD:
        {
            ...
            /*判断长度是否超限*/
            if (mAddrTarget.nAddrValue < ADDR_MAX_VALUE_8000)
            {
                return false;
            }
            if(mAddrTarget.nAddrValue + mAddrTarget.nAddrLen - ADDR_MAX_VALUE_8000 > ADDR_MAX_VALUE_512)
            {
                return false;
            }
    ...
    case REG_TYPE_SM:
        {
            ...
            /*不能为负数*/
            if(nStartValue < 0 || nStartValue >= 8512)
            {
                return ILLEGAL_VALUE;
            }
            ...
            for(int i = 0; i < nCopyNum; i++)
            {
                ...
                if(nStartValue + mTmpAddr.nAddrLen <= 8512)
                {
                    /*添加整数部分*/
                    memset(buf, 0, 10);
                    sprintf(buf, "%d", nStartValue);
    ...
    case REG_TYPE_C:
        {
            ...
            {
            case REG_TYPE_X:
                iOffsetAdd = 0x0000;                                        //0000H~00FFH X0~X377 256 点
                iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            case REG_TYPE_Y:
                iOffsetAdd = 0x0100;                                        //0100H~01FFH Y0~Y377 256 点
                iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            case REG_TYPE_M:
                if (mAddrInfo.nAddrValue <= 1535)
                {
                    iOffsetAdd = 0x0200;                                    //0200H~07FFH M0~M1535 1536 点
                    iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd;           //加上偏移地址
                }
                else
                {
                    iOffsetAdd = 0x1D00;                                    //1D00H~34FFH M1536~M7679 6144 点
                    iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd - 1536;    //加上偏移地址
                }
                break;
            case REG_TYPE_SM:
                iOffsetAdd = 0x0F00;                                        //0F00H~10FFH M8000~M8511 512 点
                iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            case REG_TYPE_S:
                iOffsetAdd = 0x0800;                                        //0800H~0BFFH S0~S1023 1024 点
                iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            case REG_TYPE_T:
                iOffsetAdd = 0x0C00;                                        //0C00H~0DFFH T0~T511 512 点
                iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            case REG_TYPE_C:
                iOffsetAdd = 0x0E00;                                        //0E00H~0EFFH C0~C255 256 点
                iSendAdd = mAddrInfo.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            default:
                iOffsetAdd = 0x0100;                                        //0100H~01FFH Y0~Y377 256 点
    ...
    case REG_TYPE_CV32:
        {
            ...
            {
            case REG_TYPE_D:
                iOffsetAdd = 0x4338;                                        //4338H~6277H D0~D7999 当前值 16000字节
                nAddrValue = mAddrInfo.nAddrValue + iOffsetAdd;
                break;
            case REG_TYPE_SD:
                iOffsetAdd = 0x6278;                                        //6278H~6477H D8000~D8511 当前值 1024字节
                nAddrValue = mAddrInfo.nAddrValue + iOffsetAdd;
                break;
            case REG_TYPE_TV:
                iOffsetAdd = 0x4000;                                        //4000H~41FFH T0~T511 当前值 1024字节
                nAddrValue = mAddrInfo.nAddrValue + iOffsetAdd;
                break;
            case REG_TYPE_CV:
                iOffsetAdd = 0x4200;                                        //4200H~42C7H C0~C199 当前值 400字节
                nAddrValue = mAddrInfo.nAddrValue + iOffsetAdd;
                break;
            case REG_TYPE_CV32:
                iOffsetAdd = 0x42C8;                                        //42C8H~4337H C200~C255 当前值 224字节
                nAddrValue = mAddrInfo.nAddrValue*2 + iOffsetAdd;
                break;
            default:
                iOffsetAdd = 0x4338;                                        //4338H~6277H D0~D7999 当前值 16000字节
                nAddrValue = mAddrInfo.nAddrValue + iOffsetAdd;
            ...
            /*算取得地址长度*/
            if (eRegType == REG_TYPE_CV32)
            {
                nAddrLen = (mAddrInfo.nAddrLen - 1) * 2;
                if (nAddrLen <= 0)
                {
                    nAddrLen = 2;
                }
    ...
    case REG_TYPE_C:
        {
            ...
            case REG_TYPE_X:
                iOffsetAdd = 0x0000;                                        //0000H~00FFH X0~X377 256 点
                iSendAdd = mAddrInfo.mAddrProp.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            case REG_TYPE_Y:
                iOffsetAdd = 0x0100;                                        //0100H~01FFH Y0~Y377 256 点
                iSendAdd = mAddrInfo.mAddrProp.nAddrValue + iOffsetAdd;               //加上偏移地址
                break;
            case REG_TYPE_M:
                if (mAddrInfo.mAddrProp.nAddrValue <= 1535)
                {
                    iOffsetAdd = 0x0200;                                    //0200H~07FFH M0~M1535 1536 点
                    iSendAdd = mAddrInfo.mAddrProp.nAddrValue + iOffsetAdd;           //加上偏移地址
                }
                ...
                {
                    iOffsetAdd = 0x1D00;                                    //1D00H~34FFH M1536~M7679 6144 点
                    iSendAdd = mAddrInfo.mAddrProp.nAddrValue + iOffsetAdd - 1536;    //加上偏移地址
                }
                break;
            case REG_TYPE_SM:
                iOffsetAdd = 0x0F00;                                        //0F00H~10FFH M8000~M8511 512 点
    ...
    case REG_TYPE_CV32:
        {
            ...
            case REG_TYPE_CV32:
                iOffsetAdd = 0x42C8;                                        //42C8H~4337H C200~C255 当前值 224字节
                iSendAdd = mAddrInfo.mAddrProp.nAddrValue*2 + iOffsetAdd;
                break;
            default:
                iOffsetAdd = 0x4338;                                        //4338H~6277H D0~D7999 当前值 16000字节
                iSendAdd = mAddrInfo.mAddrProp.nAddrValue + iOffsetAdd;
...
bool PlcProtocol::sortOutAddrList(vector<ADDR_TARGET_PROP> mSourceAddrList, vector<ADDR_TARGET_PROP> &targetList, int nMaxRWlen, bool bWriteAddr)
{
    ...
    for(int i = 0; i < nTotalSize; i++)
    {
        ...
        case REG_TYPE_M:
            {
                if (mTmpAddr.nAddrValue <= 1535 && mTmpAddr.nAddrValue + mTmpAddr.nAddrLen <= 1536)
                {
    ...
    mTmpListBit7 = PlcMathMethod::addrBubbleSort(mTmpListBit7);
    if(bWriteAddr)
    {
        /*写地址，则只支持长度为1的写*/
        mTmpListBit7 = PlcMathMethod::addrSortOut(mTmpListBit7, 1, 1);
    }
    else
    {
        /*读地址，可支持多个长度*/
        mTmpListBit7 = PlcMathMethod::addrSortOut(mTmpListBit7, nMaxRWlen, 8);
    }
    ...
    mTmpListDWord0 = PlcMathMethod::addrSortOut(mTmpListDWord0, nMaxRWlen, 1);
