    ...
    case REG_TYPE_CV32:
        {
            ...
            else      //如果是从PLC读取的数据
            {
                for(int i = 0; i < nListSize/4; i++)
                {
                    ...
                    /*设置值 4 0 12 8*/
                    int nChar1 = (valueList.at(4*i + 1) << 0) & 0xff;
                    int nChar2 = (valueList.at(4*i + 0) << 8) & 0xff00;
                    int nChar3 = (valueList.at(4*i + 3) << 16) & 0xff0000;
                    int nChar4 = (valueList.at(4*i + 2) << 24) & 0xff000000;

                    nTmpValue = nChar1 + nChar2 + nChar3 + nChar4;
                    readDataList.regDoubleWordList[0][mAddr.nAddrValue + i].nValue = nTmpValue;
                }
            }
            break;
