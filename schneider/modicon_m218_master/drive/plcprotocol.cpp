    ...
    case REG_TYPE_MX:
        {
            ...
            if(strList.size() == 2)
            {
                ...
                /*小数部分不能超过16，也不能为负数*/
                if(nIntValue < 0 || nIntValue >= 131072 || nDecValue < 0 || nDecValue >= 8)
                {
                    return false;
                }
                ...
                /*判断长度是否超限*/
                if(mAddrTarget.nAddrValue + mAddrTarget.nAddrLen > 131072 * 8)
                {
                    return false;
                }
                return true;
    ...
    case REG_TYPE_MX:
        {
            ...
            if(strList.size() == 2)
            {
                ...
                for(int i = 0; i < nCopyNum; i++)
                {
                    ...
                    if(nStartValue + mTmpAddr.nAddrLen < 131072*8)
                    {
                        /*添加整数部分*/
                        memset(buf, 0, 10);
...
bool PlcProtocol::getAddrCanRead(ADDR_TARGET_PROP mAddr)
{
    ...
    case REG_TYPE_MX:
        {
            if(mAddr.nAddrValue < 0 || mAddr.nAddrValue + mAddr.nAddrLen > 131072 * 8)
            {
                return false;
            }
            return true;
...
bool PlcProtocol::getRegIndexFromName(string sRegName, int &nRegIndex)
{
    ...
    if(DealWithString::compare("%MX", sRegName))
    {
        nRegIndex = 0;
        return true;
    }
